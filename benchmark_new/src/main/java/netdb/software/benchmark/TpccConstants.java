...
public class TpccConstants {
	...
	public static final int DISTRICTS_PER_WAREHOUSE;
	public static final int CUSTOMERS_PER_DISTRICT;
	/*
	 * WARNNING: if the customer number less than 1000, system can not generate
	 * valid constant for NURand().
	 */
	public static final int NEW_ORDERS_PER_DISTRICT;
	public static final int NEW_ORDER_START_ID;
	public static final int NUM_DISTINCT_CLAST;

	static {
		String prop = System.getProperty(TpccConstants.class.getName()
				+ ".NUM_ITEMS");
		NUM_ITEMS = (prop == null ? 100000 : Integer.parseInt(prop.trim()));
		prop = System.getProperty(TpccConstants.class.getName()
				+ ".NUM_WAREHOUSES");
		NUM_WAREHOUSES = (prop == null ? 1 : Integer.parseInt(prop.trim()));
		prop = System.getProperty(TpccConstants.class.getName()
				+ ".DISTRICT_PER_WAREHOUSE");
		DISTRICTS_PER_WAREHOUSE = (prop == null ? 10 : Integer.parseInt(prop
				.trim()));
		prop = System.getProperty(TpccConstants.class.getName()
				+ ".CUSTOMER_PER_DISTRICT");
		CUSTOMERS_PER_DISTRICT = (prop == null ? 3000 : Integer.parseInt(prop
				.trim()));
		prop = System.getProperty(TpccConstants.class.getName()
				+ ".NEW_ORDERS_PER_DISTRICT");
		NEW_ORDERS_PER_DISTRICT = (prop == null ? 3000 : Integer.parseInt(prop
				.trim()));
		NEW_ORDER_START_ID = (int) (NEW_ORDERS_PER_DISTRICT * ((double) 2101 / 3000));
		NUM_DISTINCT_CLAST = (int) NEW_ORDERS_PER_DISTRICT / 3;
	}
	...
	static {
		String prop = System.getProperty(TpccConstants.class.getName()
				+ ".FREQUENCY_TOTAL");
		FREQUENCY_TOTAL = (prop == null ? 100 : Integer.parseInt(prop.trim()));
		prop = System.getProperty(TpccConstants.class.getName()
				+ ".FREQUENCY_NEW_ORDER");
		FREQUENCY_NEW_ORDER = (prop == null ? 45 : Integer.parseInt(prop.trim()));
		prop = System.getProperty(TpccConstants.class.getName()
				+ ".FREQUENCY_PAYMENT");
		FREQUENCY_PAYMENT = (prop == null ? 43 : Integer.parseInt(prop.trim()));
		prop = System.getProperty(TpccConstants.class.getName()
				+ ".FREQUENCY_ORDER_STATUS");
		FREQUENCY_ORDER_STATUS = (prop == null ? 4 : Integer.parseInt(prop.trim()));
		prop = System.getProperty(TpccConstants.class.getName()
				+ ".FREQUENCY_DELIVERY");
		FREQUENCY_DELIVERY = (prop == null ? 4 : Integer.parseInt(prop.trim()));
		prop = System.getProperty(TpccConstants.class.getName()
				+ ".FREQUENCY_STOCK_LEVEL");
		FREQUENCY_STOCK_LEVEL = (prop == null ? 4 : Integer.parseInt(prop.trim()));
	}
